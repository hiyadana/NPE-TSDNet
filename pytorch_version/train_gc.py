...
from pytorch_version.models import G_Net
...
def train_g_net():
    ...
    model_path = os.path.join(model_save_dir, "g_model.pth")

    # --- 2. 데이터셋 및 데이터로더 준비 ---
    print("Loading G-Net pre-training data...")
    try:
        train_dataset = NucDataset(data_path=train_path, mode='pretrain_g')
        # num_workers > 0 으로 설정하여 데이터 로딩 속도를 높일 수 있다.
    ...
    # 원본 코드의 학습률 스케줄러와 유사하게 25 에포크에서 학습률을 1/10로 줄인다.
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=25, gamma=0.1)

    # --- 4. 학습 루프 ---
    print("Starting G-Net pre-training...")
    ...
    print(f"G-Net model saved to {model_path}")
